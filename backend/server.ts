...
app.get("/api/products",(req:any,res:any)=>{
    getConnection().then((conn:any)=>{
        let db:any = conn.db("mernstack");
        db.collection("prod").find().toArray((err:any,array:any)=>{
            if(err) throw err;
            else{
                res.send(array);
            }
...
app.get("/api/products/:id",(req:any,res:any)=>{
      getConnection().then((conn:any)=>{
        let db:any = conn.db("mernstack");
            ...
            try{
                db.collection("prod").find({"_id":new obj.ObjectId(req.params.id)}).toArray((err:any,array:any)=>{
                    if(err) throw err;
                    else{
                        if(array.length>0){
                            res.send(array[0]);
